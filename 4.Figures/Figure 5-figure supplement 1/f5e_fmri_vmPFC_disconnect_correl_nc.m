...
outfile = fullfile(roi_folder, 'plot', 'plot_correl_gap_vmPFC_nc.pdf');



%% !UPDATE! Data [nP x 1]
load(fullfile(folder, '6.group', 'covariates.mat')); 
data1 = normalize(pc_nc_nan) - normalize(delta_nc);


load(fullfile(roi_folder, 'results', 'f2_v26.mat')); 
data2 = result(:,2); %y = data2



%% !UPDATE! Labels
xL = 'Belief disconnection (U)';
%xL = ['Discrepancy in belief' sprintf('\n controllability (C)')];
yL = 'vmPFC coefficient (U)';
% to change the line:
...
set(gca, 'box', 'off', 'FontName', 'Arial', 'FontSize', 8, ...
    'xLim', xlim, 'yLim', ylim, 'TickLength', [0 0], 'xtick', [-5 0 5], 'ytick', [-2 0 2 4]);
