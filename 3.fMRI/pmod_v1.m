...
outdir = 'C:\Users\SN\Google Drive\p1.UG2_fMRI\180422_eta.8\4.pmod\v1';
mkdir(outdir);
...
for i = 1:length(ID)
    [IC_chosenValue{i}, IC_normViolation{i}] = ...
        nRv_f3_cap2_t20_etaf_values(OFFER{i}, RESP{i}, fixed, param(i,:,4));        
end
...
for i = 1:length(A)  
    ...
    if A(i).order == 1    % IC first, NC second
            ...
            pmod.block1 = {IC_chosenValue{i}, IC_normViolation{i}};
            pmod.block2 = {NC_chosenValue{i}, NC_normViolation{i}};

        elseif A(i).order == 2    % NC first, IC second

            pmod.block1 = {NC_chosenValue{i}, NC_normViolation{i}};
            pmod.block2 = {IC_chosenValue{i}, IC_normViolation{i}};
